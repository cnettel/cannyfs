...
static int cannyfs_getattr(const char *path, struct stat *stbuf)
{
	...
	if (inaccurate)
	{
		if (options.verbose && b.fileobj) fprintf(stderr, "Inner file obj %llx for %s, missing status %d, would report \n", b.fileobj, path, (int)b.fileobj->missing, (int)(options.cachemissing && b.fileobj && b.fileobj->missing));
